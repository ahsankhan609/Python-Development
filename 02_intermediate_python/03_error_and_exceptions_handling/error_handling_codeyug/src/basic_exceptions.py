...
try:
    ...
    # code that will be executed if no exception is raised and the try block is executed successfully
    print("No exception occurred. from else block.")
finally:
    # code that will be executed(always) regardless of whether an exception is raised or not. for example: closing a file, closing a database connection, etc.
...
try:
    ...
    # we are raising an exception manually, so it will raise an exception and go to except block
    ...
    # code that will be executed if no exception is raised and the try block is executed successfully
    print("No exception occurred. from else block. No error in try block. So this block will not be executed.")
finally:
    # code that will be executed(always) regardless of whether an exception is raised or not. for example: closing a file, closing a database connection, etc.
    print("Finally block executed. from finally block. Even if there is an exception, this block will be executed.")

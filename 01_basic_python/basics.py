...
PI = 3.1415
API_KEY = "hdsfas2439387eoiyqyr326975" # we can change it anytime, but we should follow naming convention and don't change it
...
formated_big_number:int = 1_000_000 # when printed, it is ignored by interpreter
...
another_cart:list[int | float] = [10,20.25,30,40.45,50,60.65] # list of integers and float
...
#TODO: turn on text wrap settings
print(f"List of set with unique numbers :{set_of_numbers}. \n List of frozen set with unique numbers :{set_of_numbers}")# when we print it in terminal, it will only print unique values if there are same values. and don't keep the order in which we give it data
...
dict_of_users:dict[str, str] = dict(user1="John", user2="Smith", user3="Mario") # another way to write dictionary
...
modulo_nums:int = num_1 % num_2 # it means when we divide one number to other, what will be remaindered
print(modulo_nums)
